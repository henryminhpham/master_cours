 ...
 */
void restart(void) {
  struct callo *p1 = &v.ve_callout[0];
  int i = 0;  
  // On lance toutes les fonctions dans le c_time <= 0 et on compte le nb de fonctions lancées
  while(p1->c_func != 0 && p1->c_time <= 0) {
    p1->c_func(p1->c_arg);
    p1++;
    i++;
  }
  // Si aucune fonction n'a été lancé
  if(i == 0) {
    return;
  }
  MASK_ALRM;
  struct callo *p2 = &v.ve_callout[0];
  while(p1->c_func != 0) {
    (p2)->c_time = p1->c_time;
    (p2)->c_func = p1->c_func;
    (p2)->c_arg = p1->c_arg;
  ...
  int j;
  p2 = &v.ve_callout[v.v_hsize - i];
  for(j = v.v_hsize - i ; j < v.v_hsize ; j++) {
    (p2)->c_time = 0;
    (p2)->c_func = 0;
    (p2)->c_arg = NULL;
    (p2)->c_cid = 0;
...
int timeout(void (*fun)(), void *arg, int tim){ 
  ...
  if(p2 == &v.ve_callout[MAX_CALLOUT - 1]) {
    perror("Dépassement du tableau");
    return -1;
  } 
  while(p2 >= p1) {
...
int untimeout(int ident) {
  ...
  if(p1 == &v.ve_callout[v.v_hsize -1] && p1->c_cid  != ident) {
    return -1;
  }
  printf("Trouvé %d pour %d \n", p1->c_cid, ident);
  v.v_hsize -= 1; 
 struct callo *p2 = p1 + 1;
 while(p2->c_func != 0) {
    (p2 - 1)->c_time = p2->c_time;
    (p2 - 1)->c_func = p2->c_func;
    (p2 - 1)->c_arg = p2->c_arg;
